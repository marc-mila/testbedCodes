...
def read_arduino_traffic_light_status(arduino, flag):
    while True:
        lectura = arduino.readline().decode().rstrip()
        if len(lectura) == 11:
            sem1, sem2 = lectura.split('/')
            colsem1 = sem1[-4:]
            colsem2 = sem2[-4:]
            traffic_light_status[str(flag) + str(sem1[0])
                                 ] = traffic_light_color[colsem1]
            traffic_light_status[str(flag) + str(sem2[0])
                                 ] = traffic_light_color[colsem2]
            #print(str(traffic_light_status))
            # if frontend:
            #    frontend.sendStatus(traffic_light, status)


def read_traffic_light_status():
    thread = Thread(target=read_arduino_traffic_light_status,
                    args=(arduinos[0], 'TW'))
    thread.start()
    thread1 = Thread(target=read_arduino_traffic_light_status, args=(arduinos[1], 'TS'))
    thread1.start()
...
def receive_request():
    while True:
        for agent in agents:
            try:
                ...
                if(msg != None and msg != ""):
                    ...
                    if msg == TRAFFIC_LIGHT_REQUEST:
                        info = json.dumps(traffic_light_dict,
                                          ensure_ascii=False)
			...
			traffic_info = msg.split("_")[2]
                        info = RESPONSE_TRAFFIC_LIGHT_COLOR+"_" + traffic_light_status[traffic_light] + "_" + traffic_info
...
def load_nested_leaders():
    ...
    nested_leaders = {}
    for leader in data:
        nested_leaders[leader[0]] = (
            leader[1].split(',')[0], leader[1].split(',')[1])
...
def on_receive_status(*args):
    data = args[0]
    print(args)
    print("Recibido cambio de estado para: " +
          data["agente_id"]+". Estado: "+data["status"])
...
if __name__ == "__main__":
    try:
        ...
        #read_arduino_traffic_light_status(arduinos[0], 'TW')
        #read_arduino_traffic_light_status(arduinos[1], 'TS')
    except Exception as e:
